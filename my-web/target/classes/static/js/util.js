...
$(document).ajaxError(function (event, jqXHR, ajaxOptions, thrownError) {
	if(null!=jqXHR.responseText){
		if(jqXHR.responseText.indexOf('登录页')>=0){
			openAlert(0,"提示","登录超时，请重新登陆！",function(){
				location.reload();
			});
		}else{
			openAlertMsg("服务器异常，请刷新后再试！");
		}
	}else
		openAlertMsg("服务器异常，请刷新后再试！");
});
...
function bindFileEvent(){
	var selector="input[type='file'][exp]";
	var fileCount=$(selector).length;
	if(fileCount==0)
		return false;
	//<input imgid="img" hidid="hidurl" upurl="upload/contract" id="fileContract" type="file" multiple="">
	$(selector).change(function() {
		var fileId=$(this).attr("id");
		if((fileId || "")==""){
			log("上传按钮的 fileId 为 undefined");
			return;
		}
		var isupfile=$(this).attr("upfile");
		
		var exp=$(this).attr("exp");
		if((exp || "")==""){
			log("上传按钮的 exp 为 undefined");
			return;
		}
		var url=$(this).attr("upurl");
		if(url.length==0){
			alert("file input must set url prop!");
		}
		if(typeof(isupfile)=="undefined")
			isupfile="0";
		if(isupfile=="1" || isupfile=="true" || isupfile==true){
			uploadFile(fileId, url,null, function(data) {
				layer.close('loading');
				if (data.length == 0) {
					openAlert(1, "提示", "上传失败");
				} else {
					$.each(data, function(index, item) {
						//var localUrl = item.pathUrl;
						var imgUrl=item.fwUrl==null?'':item.fwUrl;
						//var fun=exp+"='"+imgUrl+"'";
						var fun=exp.replace(/\?/g,imgUrl);
						//log(fun);
						eval(fun);
			...
			uploadImage(fileId, url,null, function(data) {
				layer.close('loading');
				if (data.length == 0) {
					openAlert(1, "提示", "上传失败");
				} else {
...
function uploadImage(fileId,src,param,callback){
	...
	var fileObj = document.getElementById(fileId).files; // 获取文件对象
	 ...
	 var filepath=$("#"+fileId).val();
	  var extStart=filepath.lastIndexOf(".");
	  var ext=filepath.substring(extStart,filepath.length).toUpperCase();
	  if(ext!=".JPEG"&&ext!=".JPG"&&ext!=".GIF"&&ext!=".AI"&&ext!=".PDG"&&ext!=".PNG"&&ext!=".BMP"){
		  layer.close(index);
		  layer.alert("请选择正确的图片格式!");
		  return false;
	  }
...
function init() {
	...
	$.post("/depart/getRootOrg",function (res) {
		if (res.success){
			var zNodes = [];
